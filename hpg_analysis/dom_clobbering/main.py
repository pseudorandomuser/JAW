...
def get_document_append_child_sinks(tx):
    ...
    return [(r['expr_node'], get_top_obj(tx, r['args_node'])['Code']) for r in tx.run(query)]
...
def get_json_sinks(tx):
    query = '''MATCH   (decl_node {Type: "VariableDeclaration"})
        ...
        RETURN decl_node, args_node;
                '''
    return [(r['decl_node'], get_top_obj(tx, r['args_node'])['Code']) for r in tx.run(query)]
...
def get_storage_sinks(tx, storage_type):
    ...
    for result in tx.run(query):
        ...
        for arg in [get_top_obj(tx, result['arg_node_key']), get_top_obj(tx, result['arg_node_val'])]:
