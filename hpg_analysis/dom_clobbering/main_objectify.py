...
class DOMClobberingAnalyzer:

    def __init__(self, args):
        self.args = args
        self.jaw_data = args.jaw_data
        self.path = args.path
        self.id = args.id
        self.url = args.url
        self.url_id = args.url_id
    ...
    ###################################'''
        ...
        vulnerability_str = ''
        for sink, code, location in self.vulnerabilities:
        ...
        vulnerabilities_len = len(self.vulnerabilities)
    ...
    def run_generic_analysis(self, tx, label, fn, args=()):
        vulnerabilities = []
        ...
        with database.session() as session:
            with session.begin_transaction() as tx:
                ...
                for params in generic_sinks:
                    self.vulnerabilities += run_generic_analysis(tx, *params)
    ...
    def generate_graph(self, program_folder_name):
    ...
    def generate_all_graphs(self, jaw_data):
        if not jaw_data:
            return
    ...
    def import_site_data(self, site_path, site_id=0, site_url=None, jaw_data=None, use_url_id=False, generate_only=False, overwrite=False):
        ...
        node_path = os.path.join(program_path_name, constants.NODE_INPUT_FILE_NAME)
        rels_path = os.path.join(program_path_name, constants.RELS_INPUT_FILE_NAME)
        ...
        if not generate_only:
            print('Importing graph into database...')
...
if __name__ == '__main__':
    ...
    main_parser.add_argument('--jaw_data', metavar='path', default=jaw_data_path, help='Path to JAW site data')
    ...
    analyzer = DOMClobberingAnalyzer(args)

    if args.action == 'analyze':
        analyzer.run_analysis()
    elif args.action == 'import':
        analyzer.import_site_data()
